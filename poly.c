...
void polyPrint(double a[], int n){
  ...
  //if the value is 0 dont print it and the value is not 1
      if(a[i]!=1&&abs(a[i])>=0)
        // if the vlaue is greater than 0 than print it
        if(a[i]>0&&a[i]!=1)
          printf("%g",a[i]);
          //if the value is less than 0 and its the frist iteration
        else if(a[i]<0&&i==n){
          if(a[i]==-1)
            printf("-");
          else printf("%g",abs(a[i]));}
      ...
      if(i>0&&abs(a[i])>0&&abs(a[i-1])>0&&checkifempty(a,i)){
        if(a[i]<0)
          printf(" - ");
        else if(a[i]>0)
          printf (" + ");
